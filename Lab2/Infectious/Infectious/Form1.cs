...
namespace Infectious
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            using (SqlConnection Conn = new SqlConnection(DB))
            {
                ...
                if (!checker1 || !checker2)
                {
                    ...
                    if (!checker1)
                    {
                        cmd.CommandText = "insert Infectious.dbo.Categorie_of_infectious_disease " + " values (N'Епідемії', N'масове поширення інфекційної хвороби серед населення відповідної території за короткий проміжок часу')";
                        cmd.ExecuteNonQuery();
                    }
                    if (!checker2)
                    {
                        cmd.CommandText = "insert Infectious.dbo.Categorie_of_infectious_disease " + " values (N'Грибкові захворювання‎', N'Опис')";
                        cmd.ExecuteNonQuery();
                    }
                }
            }
        ...
        public void Refresh_ComboBox()
        {
            ...
            {
                Conn.Open();
                SqlDataAdapter da = new SqlDataAdapter("select * from Infectious.dbo.Categorie_of_infectious_disease", DB);
                DataSet ds = new DataSet();
                da.Fill(ds);
            ...
            if (radioButton1.Checked)
            {
                if (checkBox1.Checked)
                {
                    ...
                    if (id != null)
                    {
                        ...
                        {
                            Conn.Open();
                            SqlDataAdapter da = new SqlDataAdapter("select * " + " from Infectious.dbo.Infectious_disease as I_d " + " where I_d.Categorie_ID = " + id, DB);
                            DataSet ds = new DataSet();
                            da.Fill(ds);
                ...
                else if (checkBox2.Checked)
                {
                    ...
                    if (int.TryParse(textBox1.Text, out id) && id >= 1)
                    {
                        ...
                        {
                            Conn.Open();
                            SqlDataAdapter da = new SqlDataAdapter("select * " + "from Infectious_disease AS I_d " + " where I_d.ID = " + textBox1.Text, DB);
                            DataSet ds = new DataSet();
                            da.Fill(ds);
                            dataGridView1.DataSource = ds.Tables[0];
                        }
                ...
                else
                {
                    ...
                    {
                        Conn.Open();
                        SqlDataAdapter da = new SqlDataAdapter(" select * " + "from Infectious_disease AS I_d " + " INNER JOIN Infectious.dbo.Categorie_of_infectious_disease as CoID " + " on I_d.Categorie_ID = CoID.ID", DB);
                        DataSet ds = new DataSet();
                        da.Fill(ds);
                        dataGridView1.DataSource = ds.Tables[0];
                    }
            ...
            else if (radioButton2.Checked)
            {
                ...
                if (errorProvider1.GetError(textBox2) == "" && errorProvider2.GetError(textBox3) == "" && errorProvider3.GetError(textBox4) == "" && errorProvider4.GetError(textBox5) == "" && errorProvider5.GetError(comboBox1) == "")
                {
                    using (SqlConnection Conn = new SqlConnection(DB))
                    {
                        Conn.Open();
                        SqlCommand cmd = Conn.CreateCommand();
                        cmd.CommandText = "insert Infectious.dbo.Infectious_disease " + " values (N'" + textBox2.Text + "', N'" + textBox3.Text + "', N'" + textBox4.Text + "', N'" + textBox5.Text + "', " + categorie_id + ")";
            ...
            else if (radioButton3.Checked)
            {
                ...
                if (errorProvider1.GetError(textBox2) == "" && errorProvider2.GetError(textBox3) == "" && errorProvider3.GetError(textBox4) == "" && errorProvider4.GetError(textBox5) == "" && errorProvider6.GetError(comboBox1) == "" && errorProvider5.GetError(textBox1) == "")
                {
                    using (SqlConnection Conn = new SqlConnection(DB))
                    {
                        Conn.Open();
                        SqlCommand cmd = Conn.CreateCommand();
                        cmd.CommandText = "update Infectious.dbo.Infectious_disease " + " set Name = N'" + textBox2.Text + "', ThreatLevel = N'" + textBox3.Text + "', IncubatoryPeriod = N'" + textBox4.Text + "', WayOfInfection = N'" + textBox5.Text + "', Categorie_ID = " + comboBox1.Text + " where ID = " + textBox1.Text;
                ...
                else
                {
                    if (int.TryParse(textBox1.Text, out id) && id >= 1)
                    {
                        ...
                        using (SqlConnection Conn = new SqlConnection(DB))
                        {
                            Conn.Open();
                            SqlCommand cmd = Conn.CreateCommand();
                            cmd.CommandText = "delete from Infectious.dbo.Infectious_disease " + " where ID = " + textBox1.Text;
